...
| **RED-CNN (this model)** | **22.27 ± 1.58** | **0.775 ± 0.049** |

*(Classical-baseline row and RED-CNN row above are computed on the same 200-slice matched subsample, for a fair comparison — non-local means is slow enough that running it on all 1136 test slices wasn't practical. RED-CNN's own full-test-set number, 1136 slices, is 22.26 ± 1.57 dB / 0.774 ± 0.049 — consistent with the subsample.)*

**RED-CNN clearly beats both classical baselines, but not by the same margin on both metrics.** It beats the better classical method on PSNR (Gaussian, 20.82 dB) by 1.45 dB, and beats the better classical method on SSIM (NLM, 0.751) by 0.024 — NLM in particular gets respectably close on structural similarity even though it's well behind on PSNR.
...
| L506 | 526 | 17.42 → 22.83 dB | 0.777 → 0.803 | +5.41 dB |
...
What a more rigorous version would add: a perceptual or adversarial loss term to address over-smoothing; full leave-one-patient-out cross-validation; a downstream task check (e.g., does denoising change measured lesion size/density) rather than only pixel-level metrics; independent low-dose acquisitions rather than simulated ones; and a wider architecture search.

## Repository contents

- `red_cnn_ldct_denoising.py` — training + evaluation script. Upload directly to a Kaggle Script kernel; Kaggle splits it into cells automatically from the `# %%` markers, no conversion step needed.
- `red_cnn_ldct_denoising.ipynb` — the executed notebook exported from Kaggle (with output), included so results are visible without needing to run anything.
- `training_curves.png` — training loss and validation PSNR/SSIM vs. epoch
- `comparison.png` — typical-slice and worst-slice qualitative comparison (low-dose / RED-CNN / full-dose)
